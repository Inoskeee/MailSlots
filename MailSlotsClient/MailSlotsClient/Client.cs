...
namespace MailSlots
{
    public partial class frmMain : Form
    {
        ...
        private string mailSlotAddress;
        // конструктор формы
        public frmMain()
        {
            InitializeComponent();
            ...
            mailSlotAddress = $"\\\\*\\mailslot\\{tbMailSlot.Text}";
        }
        ...
        private void btnConnect_Click(object sender, EventArgs e)
        {
            try
            {
                if (!string.IsNullOrEmpty(loginText.Text))
                {
                    ...
                    HandleMailSlot = DIS.Import.CreateFile(mailSlotAddress, DIS.Types.EFileAccess.GenericWrite, DIS.Types.EFileShare.Read, 0, DIS.Types.ECreationDisposition.OpenExisting, 0, 0);
                    if (HandleMailSlot != -1)
                    {
